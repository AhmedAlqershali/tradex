...
**How to apply:** When PHPUnit behaves differently from its XML environment declarations in Replit, inspect sanitized `$_SERVER`, `$_ENV`, and `getenv()` values before changing application code or creating environment files.