...
@section('heading', 'نظرة عامة على لوحة التحكم')

@section('content')
    <div class="mb-8 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div class="text-right">
            <p class="text-sm leading-6 text-slate-500">نظرة فورية على سوق Tradex.</p>
            <p class="mt-1 text-xs text-slate-400">القيم تُقرأ مباشرة من قاعدة البيانات عند كل طلب.</p>
        </div>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Platform statistics">
        @php
            $cards = [
                ['label' => 'إجمالي المستخدمين', 'value' => $overview['users']['total'], 'detail' => $overview['users']['clients'] . ' عميل', 'icon_bg' => 'bg-indigo-50', 'icon_text' => 'text-indigo-600', 'icon' => '◎', 'url' => null],
                ['label' => 'إجمالي التجار', 'value' => $overview['users']['merchants'], 'detail' => $overview['users']['active_merchants'] . ' حساب نشط', 'icon_bg' => 'bg-violet-50', 'icon_text' => 'text-violet-600', 'icon' => '◈', 'url' => route('admin.merchants.index')],
                ['label' => 'المتاجر النشطة', 'value' => $overview['stores']['active'], 'detail' => $overview['stores']['total'] . ' متجر إجمالي', 'icon_bg' => 'bg-teal-50', 'icon_text' => 'text-teal-600', 'icon' => '⌂', 'url' => route('admin.stores.index')],
                ['label' => 'الطلبات', 'value' => $overview['orders']['total'], 'detail' => $overview['orders']['completed'] . ' مكتملة', 'icon_bg' => 'bg-rose-50', 'icon_text' => 'text-rose-600', 'icon' => '▤', 'url' => route('admin.orders.index')],
                ['label' => 'المبيعات المكتملة', 'value' => number_format($overview['total_sales'], 2), 'detail' => 'إيرادات السوق', 'icon_bg' => 'bg-orange-50', 'icon_text' => 'text-orange-600', 'icon' => '₪', 'url' => route('admin.orders.index', ['status' => \App\Models\Order::STATUS_COMPLETED])],
                ['label' => 'الاشتراكات', 'value' => $overview['subscriptions']['active'], 'detail' => $overview['subscriptions']['trials'] . ' تجربة', 'icon_bg' => 'bg-amber-50', 'icon_text' => 'text-amber-600', 'icon' => '◇', 'url' => route('admin.subscriptions.index')],
                ['label' => 'إجمالي العمولات', 'value' => $overview['commissions']['total'], 'detail' => $overview['commissions']['unpaid'] . ' غير مدفوعة', 'icon_bg' => 'bg-sky-50', 'icon_text' => 'text-sky-600', 'icon' => '◍', 'url' => route('admin.commissions.index')],
                ['label' => 'العمولات المدفوعة', 'value' => number_format($overview['commissions']['paid_amount'], 2), 'detail' => 'إجمالي المدفوعات المسجلة', 'icon_bg' => 'bg-emerald-50', 'icon_text' => 'text-emerald-600', 'icon' => '✓', 'url' => route('admin.commissions.index')],
            ];
        @endphp
